...
def ajouter(root, dico):
    ...
    def numero_verifier(event):
        ...
        try: # on tente d'obtenir un entier avec le contenu du champs
            ...
            if nbr in dico.keys():
                ...
                if nbr < 10: # on fait en sorte de toujours écrire les nombres avec 2 chiffres
    ...
    def requete_verifier(event):
        ...
        data = Database.database("imdb.db") # on met en route la base de données pour tester
    ...
    def validation():
        ...
        if renseignement[0] == False: # l'indice indiquer ne contient autre chose que des chiffres ou est vide
            ...
            with open("requetes/" + sql_file, "x") as fichier: # on cree un fichier .sql en écriture pour ajouter la requete
    ...
    def retour():
        ...
        """
        root.destroy()
        question() # on relance le programme
...
def supprimer(root, dico):
    """
    fonction permettant de supprimer une requête via une fenetre tkinter
    parametres:
        root, une fenetre tkinter
        dico, un dictionnaire du type {indice de la question : [question, reponse, fichier sql associé], ... }
    """
    ...
    def retour():
        ...
        elle fait redémarrer le programme
        """
        root.destroy() # on ferme la fenêtre
        question() # on relance le programme
        # cette méthode permet de rafraichir le dictionnaire contenant les informations des requetes
